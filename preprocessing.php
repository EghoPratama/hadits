...
class Preprocessing
{
	...
	function cari($kata){
		$hasil = mysqli_query(mysqli_connect('localhost', 'root', '', 'bulughul_maram'),"SELECT * FROM tb_katadasar WHERE katadasar = '".$kata."'");
	...
	//hapus first order prefiks (awalan pertama)
	function hapusawalan1($kata){
		...
		if($kelas->cari($kata)==NULL){
			if(substr($kata,0,4)=="meng"){
				...
				if(substr($kata,3,1)=="e" || substr($kata,3,1)=="o" || substr($kata,3,1)=="i" || substr($kata,3,1)=="u" || substr($kata,3,1)=="a" ){
				...
				if(substr($kata,2,1)=="r"){
					$kata = "pe".substr($kata,2);
				}
				else{
					$kata = substr($kata,2);
				}	
				...
				if(substr($kata,4,1)=="e" || substr($kata,4,1)=="a" || substr($kata,4,1)=="u"){
					$kata = "k".substr($kata,4);
				}
				else{
					$kata = substr($kata,4);
