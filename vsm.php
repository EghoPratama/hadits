...
class VSM
{
    ...
    */
    public static function get_rank($query, $dokumen)
    {
        ...
        $widf           = VSM::widf($query, $dokumen_term, $df);
        $bobot          = VSM::bobot($query, $dokumen_term, $widf);
    ...
    */
    public static function dokumen_term($dokumen)
    {
        ...
        foreach ($dokumen as $key => $value) {
            // semua string jadi array | untuk mendapatkan term
            $string_array = explode(" ", $value['hadits']);
            // mendapatkan term
            $word       = str_word_count($value['hadits'], 1); // auto string to array
            $term       = array_count_values($word);
            array_push($arrayTampung, ['id' => $value['id'], 'hadits' => $term]);
        }
        return $arrayTampung;
    }
    ...
    */
    public static function df($term, $query, $dokumen_term)
    {
        ...
        foreach ($term as $key => $value) {
            foreach ($dokumen_term as $key1 => $value1) {
                foreach ($value1['hadits'] as $key2 => $value2) {
                    if ($key == $key2) {
                        $arrayDf[$key] += 1;
                    }
                }
    ...
    /**
     * Mendapatkan nilai widf dari df
     *
     * @param  array $query
     * @param  array $dokumen_term
     * @param  array $df
     * @return array
    */
    public static function widf($query, $dokumen_term, $df)
    {
     ...
     * @return array
    */
    public static function bobot($query, $dokumen_term, $widf)
    {
        // pembobotan query
        $bobotQuery =[];
        foreach ($widf as $key => $value) {
            foreach ($query as $key1 => $value1) {
                if ($key == $value1) {
        ...
        foreach ($dokumen_term as $index => $dokumen) {
            $arrayTampung = [];
            foreach ($widf as $key => $value) {
                foreach ($dokumen['hadits'] as $key1 => $value1) {
                    if ($key == $key1) {
                        $arrayTampung += [$key => ($value*$value1),];
                    }
                }
            }
            array_push($bobotDokumen, array('id' => $dokumen['id'], "hadits" => $arrayTampung));
        }

        // Array Bobot
        $arrayBobot = ["query" => $bobotQuery, "hadits" => $bobotDokumen];
    ...
    */
    public static function cosine_similarity($bobot)
    {
        ...
        foreach ($bobot['hadits'] as $index => $dokumen) { // dokumen 1, 2, 3 ... n
            $arrayDoc[$index] = ["id" => $dokumen['id']];
            $bobotDoc[$index] = 0;
            foreach ($bobot['query'] as $key => $value) {
                foreach ($dokumen['hadits'] as $key1 => $value1) { // isi dr dokumen 1, 2, ..n
                    if ($key == $key1) {
        ...
        foreach ($dokumenVektor as $index => $dokumen) {
            $arrayDoc[$index] = ["id" => $dokumen['id'], "jumlah_vektor" => $dokumen['jumlah_vektor'] ];
            $vectorCosine[$index] = 0;

            foreach ($dokumenBesarVektor as $index1 => $dokumen1) {
                if ($dokumen1['id'] == $dokumen['id']) {
        ...
        foreach ($dokumenCosine as $index => $dokumen) {
            ...
            $arrayDoc[$index] = ["id" => $dokumen['id'], "ranking" => $jumlah];
